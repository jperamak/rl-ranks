...
const teamToCSV = (teams) => {
    let csvString =
        'Team Name;Top3 RMSQ;Team RMSQ;(top mmr + top3)/2;(top mmr + avg mmr)/2;Top 3 avg;Team avg;Avg Rank;P1;P1 MMR;P2;P2 MMR;P3;P3 MMR;P4;P4 MMR;P5;P5 MMR;\n';
    teams.forEach((team) => {
...
const fetchPlayerData = async () => {
    ...
    for (var teamIndex = 0; teamIndex < teamData.length; teamIndex++) {
        ...
        const fetchedTeam = {
            name: team[Object.keys(team)[0]], // asdf
            mmr: 0,
            players: []
        };
        process.stdout.write(
        ...
        for (let playerIndex = 1; playerIndex <= 5; playerIndex++) {
            ...
            if (player.name) {
                ...
                player.profileUrl = team[`Player ${playerIndex} RL-Tracker -profiili`];
                player.rank = team[`Player ${playerIndex} Rank (3v3)`];
                ...
                if (player.ranks[playlists.standard] === undefined)
    ...
    if (writetofile) {
        const time = Date.now();
        saveToFile(`pappa-fetchedTeams-3v3-${time}.json`, JSON.stringify(fetchedTeams));
        saveToFile(`pappa-fetchedTeams-3v3-${time}.csv`, teamToCSV(fetchedTeams));
        ...
        saveToFile(`pappa-fetchedTeams-3s2sAvg-${time}.json`, JSON.stringify(fetchedTeams));
        saveToFile(`pappa-fetchedTeams-3s2sAvg-${time}.csv`, teamToCSV(fetchedTeams));
        ...
        saveToFile(`pappa-fetchedTeams-3s2sMax-${time}.json`, JSON.stringify(fetchedTeams));
        saveToFile(`pappa-fetchedTeams-3s2sMax-${time}.csv`, teamToCSV(fetchedTeams));
    }
    const end = Date.now();
